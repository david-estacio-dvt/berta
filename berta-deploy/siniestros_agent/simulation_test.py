...
class TestCASStateMachine:
    ...
    def test_valid_initial_codes(self):
        """Códigos de inicio válidos: 100, 101, 171."""
        for code in ["100", "101", "171"]:
            assert code in CAS_TRANSITIONS["INICIO"]

    def test_transition_100_to_200(self):
        """Transición 100 → 200 (Aceptar parte inicial)."""
        result = validate_transition("100", "200")
        assert result["valid"] is True

    def test_transition_100_to_900(self):
        """Transición 100 → 900 (Rehusar parte inicial)."""
        result = validate_transition("100", "900")
        assert result["valid"] is True

    def test_transition_100_to_500_invalid(self):
        """Transición 100 → 500 no es válida (no se puede facturar sin aceptar)."""
        result = validate_transition("100", "500")
        assert result["valid"] is False

    def test_transition_200_to_328(self):
        """Transición 200 → 328 (Envío documentación tras aceptar)."""
        result = validate_transition("200", "328")
        assert result["valid"] is True

    def test_transition_500_to_600(self):
        """Transición 500 → 600 (Aceptar factura)."""
        result = validate_transition("500", "600")
        assert result["valid"] is True

    def test_transition_600_to_700(self):
        """Transición 600 → 700 (Pago tras aceptar factura)."""
        result = validate_transition("600", "700")
        assert result["valid"] is True

    def test_transition_999_terminal(self):
        """999 es estado terminal, no tiene transiciones."""
        result = validate_transition("999", "100")
        assert result["valid"] is False
    ...
    def test_code_info(self):
        ...
        assert info["nombre"] == "Parte de Asistencia Inicial"
...
class TestCASCodeSequence:
    ...
    def test_new_expediente_initial_code(self):
        result = json.loads(check_cas_code_sequence("NEW_EXP", "100"))
    ...
    def test_existing_expediente_valid(self):
        result = json.loads(check_cas_code_sequence("202400785782", "200"))
        assert result["estado"] == "VALIDO"

    def test_existing_expediente_invalid(self):
        result = json.loads(check_cas_code_sequence("202400785782", "700"))
        assert result["estado"] == "INVALIDO"
...
class TestResponseGeneration:
    ...
    def test_generate_acceptance(self):
        result = json.loads(generate_cas_response_code("202400785782", "200", "Todos los datos verificados correctamente."))
    ...
    def test_generate_invalid_transition(self):
        ...
        assert result["estado"] == "TRANSICION_INVALIDA"

    def test_generate_rehuse(self):
        result = json.loads(generate_cas_response_code("202400785782", "900", "R01"))
        assert result["estado"] == "GENERADO"
        assert result["codigo"] == "900"
...
class TestCASFlowIntegration:
    ...
    def test_full_acceptance_flow(self):
        ...
        # 1. Hospital envía parte inicial
        cas_result = json.loads(check_cas_code_sequence("NEW_EXP_001", "100"))
        assert cas_result["estado"] == "VALIDO_INICIO"
        print(f"  ✅ 1. Código 100 válido como inicio")
    ...
    def test_rehuse_flow_expired_policy(self):
        ...
        # 1. Hospital envía parte
        cas_result = json.loads(check_cas_code_sequence("NEW_EXP_002", "100"))
        assert cas_result["estado"] == "VALIDO_INICIO"
        ...
        # 3. Decisión: REHUSAR
        print(f"  🔴 Decisión: REHUSAR con código 900, motivo R01")
    ...
    def test_existing_expediente_flow(self):
        ...
        # 2. Verificar qué códigos son válidos
        # Desde código 100, podemos enviar 200, 300, 301, 900
        cas_result = json.loads(check_cas_code_sequence("202400785782", "200"))
        assert cas_result["estado"] == "VALIDO"
        print(f"  ✅ Se puede responder con 200 (Aceptar)")

        # 3. Generar respuesta
        response = json.loads(generate_cas_response_code("202400785782", "200", "Verificación completada. Datos correctos."))
        assert response["estado"] == "GENERADO"
        print(f"  📤 Respuesta generada: {response['codigo']} - {response['nombre']}")
