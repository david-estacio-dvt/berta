...
def _inferir_emisor(codigo: str) -> str:
    ...
    Prioriza CAS_CODES (fuente oficial) sobre los conjuntos estáticos.
    """
    try:
    ...
    if codigo in CAS_CODES:
        return CAS_CODES[codigo].get("emisor", "DESCONOCIDO")
...
def _extraer_datos_factura(msg_raw: dict) -> Optional[dict]:
    """
    Intenta extraer el desglose de factura del contenido de un mensaje SIAX.
    Busca en campos como Contenido, DatosDocumento, ContenidoXml, etc.
    """
    contenido_xml = None
    for campo in ("Contenido", "ContenidoXml", "DatosDocumento",
                   "ContenidoMensaje", "Xml", "Body", "Detalle"):
        val = msg_raw.get(campo)
    ...
    try:
        ...
        return {"contenido_raw": contenido_xml[:2000]}

    factura = {}

    for tag in ("ImporteTotal", "ImporteFactura", "Importe", "Total",
                 "ImporteReclamado", "ImporteNeto"):
        elem = root.find(f".//{tag}")
        if elem is not None and elem.text:
            try:
                factura["importe_total"] = float(elem.text.replace(",", "."))
                break
            except ValueError:
                pass

    prestaciones = []
    for prest_tag in ("Prestacion", "Prestaciones", "LineaFactura",
                       "Servicio", "Concepto", "DetalleFactura"):
        for elem in root.iter(prest_tag):
            prest = {}
            for child in elem:
                tag_name = child.tag.split("}")[-1] if "}" in child.tag else child.tag
                prest[tag_name.lower()] = child.text
            if prest:
                prestaciones.append(prest)

    if prestaciones:
        factura["prestaciones"] = prestaciones

    for tag in ("FechaFactura", "FechaEmision"):
        elem = root.find(f".//{tag}")
        if elem is not None and elem.text:
            factura["fecha_factura"] = elem.text
            break

    for tag in ("NumeroFactura", "NFactura", "ReferenciaFactura"):
        elem = root.find(f".//{tag}")
        if elem is not None and elem.text:
            factura["numero_factura"] = elem.text
            break
...
def _extraer_campos_adicionales(msg_raw: dict) -> dict:
    """
    Captura cualquier campo extra del mensaje SIAX que no esté en el esquema base.
    Esto permite exponer datos como posición de lesionado, matrícula, etc.
    si SIAX los incluye en el futuro.
    """
    campos_base = {
        "Codigo", "Estado", "FechaCaducidad", "ConvenioNormas",
        "MensajeReferido", "Id", "Secuencia",
    }
    extras = {}
    for k, v in msg_raw.items():
        if k not in campos_base and v is not None and v != "":
            extras[k] = v
    return extras
...
def _parsear_respuesta_cas(data: dict, id_cas: int, environment: str) -> dict:
    ...
    Filtra los códigos de respuesta por el convenio del mensaje.
    """
    try:
        from .cas_codes import get_codigos_respuesta_por_convenio
    except ImportError:
        from cas_codes import get_codigos_respuesta_por_convenio
    ...
    for sec_raw in secuencias_raw:
        ...
        for msg_raw in mensajes_raw:
            ...
            codigos_respuesta = get_codigos_respuesta_por_convenio(
                codigo, convenio_normas
            )
            if not codigos_respuesta:
            ...
            if codigo == "181":
                datos_factura = _extraer_datos_factura(msg_raw)
                if datos_factura:
                    mensaje["datos_factura"] = datos_factura

            extras = _extraer_campos_adicionales(msg_raw)
            if extras:
                mensaje["campos_adicionales"] = extras
        ...
        codigos_en_sec = [m["codigo"] for m in mensajes]
        if any(c in ("101", "171", "176") for c in codigos_en_sec):
            necesita_accion = True

        respondible = next(
            (m for m in reversed(mensajes)
             if m["puede_responder"] and m["codigos_respuesta_validos"]),
            None,
        )

        codigos_disp = []
        if respondible:
            codigos_disp = respondible["codigos_respuesta_validos"]
        elif necesita_accion and ultimo_mensaje:
            codigos_disp = ultimo_mensaje.get("codigos_respuesta_validos", [])
        ...
        secuencia = {
            ...
            "codigos_respuesta_disponibles": codigos_disp,
        }
