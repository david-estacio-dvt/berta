...
try:
    from .cas_codes import (
        ...
        get_codigos_respuesta_por_convenio, normalizar_convenio,
    )
...
def analizar_caso_siax(id_cas: int) -> str:
    ...
    resumen_secuencias = []
    convenios_detectados = set()

    for sec in result.get("secuencias", []):
        referencia = sec["referencia_cas"]
        mensajes_detalle = []

        codigos_en_sec = [m["codigo"] for m in sec.get("mensajes", [])]
        if any(c in ["101", "171", "176"] for c in codigos_en_sec):
            sec["necesita_accion"] = True

        for msg in sec.get("mensajes", []):
            info_codigo = get_code_info(msg["codigo"])
            nombre_codigo = info_codigo.get("nombre", f"Código {msg['codigo']}")
            emisor_oficial = info_codigo.get("emisor", msg["emisor"])
            convenio_msg = msg.get("convenio_normas", "")

            if convenio_msg:
                convenios_detectados.add(convenio_msg)

            codigos_filtrados = get_codigos_respuesta_por_convenio(
                msg["codigo"], convenio_msg
            )
            if not codigos_filtrados and msg["puede_responder"]:
                codigos_filtrados = msg["codigos_respuesta_validos"]

            detalle = {
                "id": msg["id"],
                "codigo": msg["codigo"],
                "nombre": nombre_codigo,
                "emisor": emisor_oficial,
                "estado": msg["estado"],
                "convenio": convenio_msg,
                "convenio_tipo": normalizar_convenio(convenio_msg),
            ...
            if msg["puede_responder"]:
                ...
                detalle["codigos_respuesta_validos"] = codigos_filtrados
            ...
            if msg.get("datos_factura"):
                detalle["datos_factura"] = msg["datos_factura"]

            if msg.get("campos_adicionales"):
                detalle["campos_adicionales"] = msg["campos_adicionales"]
        ...
        if sec["necesita_accion"]:
            codigos_disp = sec.get("codigos_respuesta_disponibles", [])
            ultimo_msg = sec.get("mensajes", [{}])[-1] if sec.get("mensajes") else {}
            conv = ultimo_msg.get("convenio_normas", "")
            if conv and codigos_disp:
                codigos_disp = get_codigos_respuesta_por_convenio(
                    sec["ultimo_codigo"], conv
                ) or codigos_disp
            ...
            resumen_sec["accion_requerida"] = (
                ...
                f"Convenio: {conv}. "
                f"Opciones de respuesta FILTRADAS por convenio: {codigos_disp}"
            )
    ...
    analisis = {
        ...
        "convenios_detectados": list(convenios_detectados),
        "secuencias": resumen_secuencias,
        "instrucciones": (
            "IMPORTANTE: Antes de recomendar CUALQUIER código de respuesta, Berta DEBE:\n"
            "1. Usar verify_insurance_policy para confirmar póliza vigente\n"
            "2. Usar verify_accident para cruzar matrícula y siniestro\n"
            "3. Usar verify_injured_person para verificar posición del lesionado\n"
            "4. Usar validate_cas_dates para coherencia temporal\n"
            "5. Solo DESPUÉS de los pasos anteriores, elegir un código de respuesta "
            "de la lista 'codigos_respuesta_validos' (ya filtrada por convenio).\n"
            "PROHIBIDO sugerir códigos que no estén en la lista filtrada por convenio.\n"
            "Si hay un mensaje 181 (factura), revisar el campo 'datos_factura' "
            "para verificar importes y desglose con check_tariffs."
        ),
    }
...
def analizar_mensaje_siax(id_cas: int, id_mensaje: int) -> str:
    ...
    convenio_normas = mensaje_encontrado.get("convenio_normas", "")
    tipo_convenio = normalizar_convenio(convenio_normas)

    codigos_filtrados = get_codigos_respuesta_por_convenio(
        codigo, convenio_normas
    )
    if not codigos_filtrados:
        codigos_filtrados = mensaje_encontrado.get("codigos_respuesta_validos", [])
    ...
    for msg in secuencia_contexto.get("mensajes", []):
        ...
        entrada_hist = {
            ...
            "emisor": msg_info.get("emisor", msg["emisor"]),
            "estado": msg["estado"],
            "convenio": msg.get("convenio_normas", ""),
            "fecha_caducidad": msg["fecha_caducidad"],
            "mensaje_referido": msg["mensaje_referido"],
        }
        if msg.get("datos_factura"):
            entrada_hist["datos_factura"] = msg["datos_factura"]
        historial.append(entrada_hist)
        if msg["id"] == id_mensaje:
            break
    ...
    analisis = {
        ...
        "convenio_normas": convenio_normas,
        "convenio_tipo": tipo_convenio,
        "mensaje": {
            "id": mensaje_encontrado["id"],
            "codigo": codigo,
            "nombre": info_codigo.get("nombre", f"Código {codigo}"),
            "emisor": info_codigo.get("emisor", mensaje_encontrado["emisor"]),
        ...
        "instrucciones": (
            f"ANTES de recomendar un código de respuesta, Berta DEBE:\n"
            f"1. Verificar póliza vigente con verify_insurance_policy\n"
            f"2. Cruzar matrícula y siniestro con verify_accident\n"
            f"3. Verificar posición del lesionado con verify_injured_person\n"
            f"4. Comprobar coherencia temporal con validate_cas_dates\n"
            f"5. Si es factura (181), revisar 'datos_factura' y usar check_tariffs\n\n"
            f"Mensaje a analizar: {codigo} ({info_codigo.get('nombre', '')})\n"
            f"Secuencia: {secuencia_contexto['referencia_cas']}\n"
            f"Convenio: {convenio_normas} ({tipo_convenio})\n"
            f"Opciones de respuesta (filtradas por convenio {convenio_normas}): "
            f"{codigos_filtrados}\n"
            f"PROHIBIDO usar códigos fuera de esta lista filtrada."
        ),
    }

    analisis["mensaje"]["posicion_lesionado"] = mensaje_encontrado.get(
        "posicion_lesionado", None
    )
    analisis["mensaje"]["notas_tramitador"] = mensaje_encontrado.get(
        "notas_tramitador", None
    )
    analisis["mensaje"]["siniestro_confirmado"] = mensaje_encontrado.get(
        "siniestro_confirmado", None
    )
    analisis["mensaje"]["importe_total"] = mensaje_encontrado.get(
        "importe_total", None
    )

    if mensaje_encontrado.get("datos_factura"):
        analisis["mensaje"]["datos_factura"] = mensaje_encontrado["datos_factura"]

    if mensaje_encontrado.get("campos_adicionales"):
        analisis["mensaje"]["campos_adicionales"] = mensaje_encontrado["campos_adicionales"]
