...
async def siax_analizar_caso(request: SiaxAnalisisCasoRequest):
    ...
    **Botón 1 — Analizar Caso Completo.**
    ...
    2. Enriquece cada mensaje con emisor oficial, necesita_accion y códigos filtrados por convenio
    ...
    _enriquecer_datos_siax(datos_siax)
...
async def siax_analizar_mensaje(request: SiaxAnalisisMensajeRequest):
    ...
    **Botón 2 — Analizar Mensaje y Recomendar Respuesta.**
    ...
    Flujo:
    1. Llama a SIAX → ObtenerCas(idCas)
    2. Enriquece mensajes con emisor oficial, necesita_accion y códigos filtrados
    3. Localiza el mensaje por su ID dentro de las secuencias
...
def _enriquecer_datos_siax(datos_siax: dict) -> None:
    """
    Enriquece in-place los datos crudos de siax_client ANTES de construir el prompt.
    Aplica los mismos parches que tools.py para que el flujo API (Postman)
    sea idéntico al flujo ADK (chat con Berta).
    """
    from cas_codes import CAS_CODES, get_codigos_respuesta_por_convenio
    ...
    for sec in datos_siax.get("secuencias", []):
        ...
        if any(c in ("101", "171", "176") for c in codigos_en_sec):
            ...
            datos_siax["secuencias_respondibles"] = max(
                datos_siax.get("secuencias_respondibles", 0), 1
            )

        for msg in sec.get("mensajes", []):
            codigo = msg.get("codigo", "")
            if codigo in CAS_CODES:
                msg["emisor"] = CAS_CODES[codigo].get("emisor", msg["emisor"])
                code_info = CAS_CODES[codigo]
                if "nombre" not in msg or not msg.get("nombre"):
                    msg["nombre"] = code_info.get("nombre", f"Código {codigo}")

            convenio = msg.get("convenio_normas", "")
            if convenio and msg.get("puede_responder"):
                filtrados = get_codigos_respuesta_por_convenio(codigo, convenio)
                if filtrados:
                    msg["codigos_respuesta_validos"] = filtrados

        if sec["necesita_accion"] and not sec.get("codigos_respuesta_disponibles"):
            respondible = next(
                (m for m in reversed(sec.get("mensajes", []))
                 if m.get("puede_responder") and m.get("codigos_respuesta_validos")),
                None,
            )
            if respondible:
                sec["codigos_respuesta_disponibles"] = respondible["codigos_respuesta_validos"]
