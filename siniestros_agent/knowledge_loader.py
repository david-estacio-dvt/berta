...
def load_all_documents(max_chars_per_doc: int = 600000) -> str:
    """
    Carga y concatena el texto de todos los documentos normativos en formato Markdown
    desde la carpeta .documentation/md/.
    ...
    if not md_dir.exists():
        print("[WARN] Carpeta .documentation/md/ no encontrada. Ejecuta conversor primero.")
        return "No se encontró documentación normativa en Markdown."
    ...
    # Archivos Markdown normativos
    files_to_process = []
    for item in sorted(md_dir.iterdir()):
        if item.is_file() and item.suffix.lower() == '.md':
            files_to_process.append(item)
    
    print(f"\n[Knowledge Loader] Procesando {len(files_to_process)} documentos normativos (Markdown)...")
    ...
    for filepath in files_to_process:
        ...
        if file_key in cache and cache[file_key].get('hash') == file_hash:
            ...
            # Leer el archivo MD directamente
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    text = f.read()
            except Exception as e:
                print(f"  [WARN] Error leyendo MD {filepath.name}: {e}")
                text = ""
